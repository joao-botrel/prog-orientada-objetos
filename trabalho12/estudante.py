...
class LimiteConsultaEstudantes(tk.Toplevel):
    def __init__(self, controle):
        ...
        self.labelMatricula = tk.Label(
            self.frameMatricula, text="Nro Matrícula: ")
...
class CtrlEstudante():
    ...
    def consultHandler(self, event):
        nroMatric = self.limiteConsulta.inputMatricula.get()
        aux = 0
        for matricula in self.listaEstudantes:
            if nroMatric == matricula.getNroMatric():
                self.limiteConsulta.mostraJanela('Sucesso', 'Nro Matric: ' + matricula.getNroMatric() + ' -- Nome: ' + matricula.getNome() + '\n')
                aux +=1
                self.clearHandler(event)
                
        if aux == 0:
            self.limiteConsulta.mostraJanela("Erro!", "Estudante não achado ou matrícula inexistente.")
            self.clearHandler(event)
