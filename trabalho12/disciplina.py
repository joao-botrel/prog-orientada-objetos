...
class LimiteConsultaDisciplina(tk.Toplevel):
    def __init__(self, controle):
        ...
        self.labelMatricula = tk.Label(self.frameCodigo, text="Codigo: ")
    ...
    def consultHandler (self, event):
        ...
        for cod in self.listaDisciplinas:
            if codigo == cod.getCodigo():
                aux += 1
                self.limiteConsulta.mostraJanela('Sucesso', 'Código: ' + cod.getCodigo() + ' -- Nome: ' + cod.getNome() + '\n')
            if aux == 0:
                self.limiteConsulta.mostraJanela("Erro!", "Disciplina não achada ou código inexistente")
                self.clearHandler(event)
